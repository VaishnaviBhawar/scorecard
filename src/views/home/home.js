...
function Home() {
  ...
  return (
    <div className='min-h-screen bg-gradient-to-r from-blue-500 to-purple-500 p-5'>
      <div className='flex items-center justify-center py-10'>
        <img src={Cardicon} alt="score card img" className='h-20 w-20 sm:h-40 sm:w-40 ' />
        <span className='text-white text-4xl sm:text-6xl font-bold'>
          ScoreMaster
        </span>
      </div>
      ...
      <div className='flex justify-around flex-wrap mt-10'>
        <Scorecards
          score={teamA}
          teamName="Team A"
          increaseScore={() => setTeamA(prevScore => prevScore + 1)}
          decreaseScore={() => setTeamA(prevScore => Math.max(prevScore - 1, 0))}
          winner={winner}
        />
        <Scorecards
          score={teamB}
          teamName="Team B"
          increaseScore={() => setTeamB(prevScore => prevScore + 1)}
          decreaseScore={() => setTeamB(prevScore => Math.max(prevScore - 1, 0))}
          winner={winner}
        />
      </div>
